...
async function addBabyInfo(id, baby) {
  const babyInfoObject = await query(
    "INSERT INTO baby (first_name, last_name, age, gender, weight, baby_picture) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *;",
    [
      baby.first_name,
      baby.last_name,
      baby.age,
...
async function updateBaby(id, baby) {
  ...
  const updateBabyObject = await query(
    "UPDATE baby SET first_name=$1, last_name=$2, age=$3, gender=$4, weight=$5 ,baby_picture=$6 WHERE id = $7 RETURNING *;",
    [
      baby.first_name,
      baby.last_name,
      baby.age,
...
async function deleteBaby(id) {
  const deleteBabyObject = await query(
    "DELETE FROM baby WHERE id = $1 RETURNING *;",
    [id]
  );
...
export { getBabyInfoByID, addBabyInfo, updateBaby, deleteBaby };
